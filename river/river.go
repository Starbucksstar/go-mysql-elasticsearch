...
func (r *River) newCanal() error {
	...
	cfg.TLSConfig = &tls.Config{InsecureSkipVerify: r.c.UseSsl}
	//cfg.Dump.ExtraOptions = []string{"--ssl-mode=REQUIRED"} --mysql5.7.11之后才有 REQUIRED == 1
	//参考链接：https://dev.mysql.com/doc/refman/5.7/en/connection-options.html#encrypted-connection-options
	cfg.Dump.ExtraOptions = []string{r.c.MysqlDumpUseSsl}
